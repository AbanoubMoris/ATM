...
struct Transaction
{
	string  Amount="0", Date="0", Type1="0";
};
struct client {
...
void EmpData(Employee BankEmpsData[]);       //to load Employees Data from file
void getdata(client User[]);                 //to get data from user
void WriteData(client User[]);               //to write the data in its file after editting
void WriteTransActs(client User[]);          //to write Transactions in its file
void ReadTransActs();                        //to Read Transactions from file
void LoadData(client User[]);                //to Load Clients Data from file
void WriteDailyRep(client User[]);           //To Write Daily Report
void VeiwReport();                           //to show Daily Report 
void IfClient(client User[]);                //User Functionalities
int EditData(client User[],int ClientNum);   //To Edit Data
void IfEmp(Employee BankEmpsData[]);         //Bank Employee Functionalities
	
void main() {
	start();
	LoadData(User);					         //load data from file
	ReadTransActs();                         //Read transaction from file
	EmpData(BankEmpsData);                   //Read Employees Data
	...
	while (true) {                           // loop to return to main screen
		...
		if (choose == 1) {
			...
			getdata(User);                       //function to get data from user
		}
		else if (choose == 3) {
			IfEmp(BankEmpsData);
		}
...
void getdata(client User[]) {                     //to get data from user                                   
	...
	//to make the first transaction in file
	User[Num_of_c].ClientTrans[0].Type1 = "Deposite";
	string s = to_string(User[Num_of_c].deposite);   //to convert int to string to be able to save int in the struct of transaction
...
int EditData(client User[],int ClientNum) {
	int Edit;
	string New = "";
